...
extension FeedItemViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if 0 < items.count {
            itemItem.configureContent(pick: pick, item: items[indexPath.row % items.count], isInteractive: true)
        } else {
    ...
    func getNewItems(offset:Int, limit: Int) {
        ...
        let subcategoryIDs: [Int] = {
            if case let .occasions(id) = pick.type {
        ...
        // If filters contains type is sale, that get items with old prices set
        let sale = pick.filters.contains(.sale)
